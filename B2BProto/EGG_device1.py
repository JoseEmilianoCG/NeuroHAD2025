...
def EEG(
    ...
    BoardShim.log_message(
        LogLevels.LEVEL_INFO.value, " ---- Starting the streaming with Muse ---"
    )
    ...
    try:
        while True:
            ...
            unix_ts = (
                t_pull
                - (len(board_ts) - 1) / sampling_rate
                + np.arange(len(board_ts)) / sampling_rate
            )
            ...
            for eeg_channel in eeg_channels:
                ...
                DataFilter.remove_environmental_noise(
                    data[eeg_channel], sampling_rate, noise_type=1
                )
                # Bandpass Filter
        ...
        BoardShim.log_message(
            LogLevels.LEVEL_INFO.value, " ---- End the session with Muse ---"
        )
