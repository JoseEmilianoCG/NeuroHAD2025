...
# # CODE FOR EEG # #
def EEG(second, folder, eno1_datach1, eno1_datach2, eno1_datach3, eno1_datach4, totaltime, muse_id):
    print(muse_id)
    ...
    params.serial_number = muse_id#'Muse-E215'# #'Muse-06D3'
    ...
    BoardShim.log_message(LogLevels.LEVEL_INFO.value, ' ---- Starting the streaming with Muse ---')
    ...
    try:
        while (True):
            ...
            for eeg_channel in eeg_channels:
                ...
                #Bandpass Filter
                DataFilter.perform_lowpass(data[eeg_channel], sampling_rate, cutoff=100, order=4, filter_type=0, ripple=0) # Se puede modificar
            ...
            df_raw.to_csv('{}/Prepro/Raw.csv'.format(folder), mode='a')
            df_clean.to_csv('{}/Prepro/Clean.csv'.format(folder), mode='a')
            
            # Calculate the new variable based on the formula // Only if you want to re-reference electrodes
            ...
            # arrange=referenced_electrodes.to_dict('dict') # If re-referenced electrodes are used
            ...
            eno1_datach1[:] = pad_or_trim(lista1, window)
            eno1_datach2[:] = pad_or_trim(lista2, window)
            eno1_datach3[:] = pad_or_trim(lista3, window)
            eno1_datach4[:] = pad_or_trim(lista4, window)
        ...
        BoardShim.log_message(LogLevels.LEVEL_INFO.value, ' ---- End the session with Muse ---')
